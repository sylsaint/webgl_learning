...
const VSHADER_SOURCE = `
  ...
  attribute mat4 uni_model_mat;
  uniform mat4 uni_mat;
  varying vec4 v_color;
  void main() {
    gl_Position = uni_mat * uni_model_mat * attr_position;
    gl_PointSize = 10.0;
    v_color = attr_color;
  }
`;
...
export default function main() {
  ...
  if (uni_model_mat < 0) {
    console.log('Failed to get the uni_model_mat storage location');
    return;
  }
  const modelMatrix = new Matrix4();
  modelMatrix.setRotate(-10, 0, 0, 1);
  gl.uniformMatrix4fv(uni_model_mat, false, modelMatrix.elements);
